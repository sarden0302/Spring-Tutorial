...
public class IndexController {
    ...
    // 주소를 엔드포인트로 지정
    /*
    Whitelabel Error Page Current request is not a multipart
    ...
    public String registerSuccess(@RequestParam("username") String username,
                                  ...
                                  MultipartFile profileImagePath, Model model) {
    ...
    @GetMapping("/find-username-result") // endpoint = api = url /find-username-result
    ...
    public String findByEmail(@RequestParam("username")String username, @RequestParam("age") String gender, Model model){
        String email = userProfileService.findByEmail(username, gender);
    ...
    public String getUser(@PathVariable int userId, Model model) {
        /* 자료형으로 service 변수명 설정할 때 어떤 자료형을 사용해야할지 모르겠다면
         0. ServiceImpl에서 해당 함수 리턴 값 확인
         1. String      2. Model      3. DTO에 작성한 클래스명칭
         ※ void형일 때 변수명 설정x
        */
    ...
    // /login 주소로 접속하면 login.html 에 작성한 화면이 보임
    @GetMapping("/login")
    public String login() {
        return "login";
    }

    /** /login 이 post로 mapping 되었기 때문에 login.html 에 작성한 username, email에 대한 결과를 확인하는 주소값
     * @RequestParam login.html 에서 form 태그 내부나 특정 태그에 작성된 name 값 중 필수로 전달되어야하는 name 명칭에 대해 작성
     * @param username = login.html 에서 name = username 으로 작성되어 있는 name 명칭
     * @param email = login.html 에서 name = email 으로 작성되어 있는 email 명칭
    ...
    public String login(@RequestParam String username, @RequestParam String email, Model model, HttpSession session) {
        ...
        // 2. 로그인에 실패했을 경우 회원정보가 null 값일 것 (왜나하면 정보가 없기 때문)
    ...
    public String logout(HttpSession session) {
        ...
        return "redirect:/"; // 로그아웃된 정보를 재설정하면서 index.html 로 돌아가기
    }

    // mypage.html로 접속할 수 있는 api
    @GetMapping("/mypage")
    public String myPage(Model model, HttpSession session) {
        /*
        @PathVariable String userId, Model model
        User usr = userProfileService.getUser(userId);
     ...
     * }
     *
     * 이 코드 작성할 필요 x -> Spring 에서 자동으로 /error.html 을 기본으로 제공
     */
}